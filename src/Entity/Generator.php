...
class Generator extends AbstractGenerator implements GeneratorInterface
{
     ...
     */
	public $tablesEnabled = [];

    /**
     * @var array $tablesDisabled 不操作的表
     */
	public $tablesDisabled = [];
     ...
     */
	public $removeTablePrefix = '';

	/**
     * @var bool $force 强制覆盖实体文件
     */
	public $force = false;
    ...
    public function execute(Schema $schema)
    {
        ...
        foreach ($tables as $table) {
	        $entityClass=$this->getEntityClass($table['name'],$this->removeTablePrefix);
	        $file=alias('@entityPath') . "/{$entityClass}.php";
	        if (!$this->force&&is_file($file)) {
	        	output()->info('Ignore exist file:'.$file);
		        continue;
	        }
